...
export default function ImpactCalculator({ colors: C }: { colors: Colors }) {
  ...
  const currentCost = people * salary * ENCARGOS
  const projectedCost = 1 * salary * ENCARGOS
  const monthlySavings = Math.max(0, currentCost - projectedCost)
  const annualSavings = monthlySavings * 12

  const monthlyDisplay = useCountUp(monthlySavings, 500)
  ...
  return (
    <div style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "26px 26px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <p style={{ fontSize: 16, color: C.muted, lineHeight: 1.7 }}>
        Isso equivale a <span style={{ color: C.text, fontWeight: 600 }}>{formatBRL(annualSavings)}</span> por ano voltando pro seu caixa.
      </p>
    </div>
  )
}
